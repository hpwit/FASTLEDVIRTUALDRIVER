...
protected:
    ...
    static void i2sInit()
    {
     ...
     //   initBitPatterns();
        ...
        dmaBuffers[0] = allocateDMABuffer((NUM_VIRT_PINS+1)*3*8*NUM_COLOR_CHANNELS*4);
        dmaBuffers[1] = allocateDMABuffer((NUM_VIRT_PINS+1)*3*8*NUM_COLOR_CHANNELS*4);
        dmaBuffers[2] = allocateDMABuffer((NUM_VIRT_PINS+1)*3*8*NUM_COLOR_CHANNELS*4);
    ...
    {
    memset((uint8_t*)buff,0,(NUM_VIRT_PINS+1)*8*4*NUM_COLOR_CHANNELS*3);
    for (int i=0;i<24*NUM_COLOR_CHANNELS;i++)
        {
   ...
   static void pu2(uint32_t* buff)
    {
     ...
     for (int j=0;j<8*NUM_COLOR_CHANNELS;j++)
      {
