...
static    void transpose16x1_noinline2(unsigned char *A, uint8_t *B) {
    ...
    x1 = 0;//*(unsigned int*)(A+12);
      ...
      // t = (x1 ^ (x1 >>14)) & 0x0000CCCC;  x1 = x1 ^ t ^ (t <<14);
        ...
        x1= ((y1 >> 4) & 0x0F0F0F0F); //(x1 & 0xF0F0F0F0) | ((y1 >> 4) & 0x0F0F0F0F);
        y1 = (y1 & 0x0F0F0F0F); //((x1 << 4) & 0xF0F0F0F0) | (y1 & 0x0F0F0F0F);
        //x1 = t;
    ...
    */
     ...
     *((uint16_t*)(B)) = (uint16_t)(   (   (x & 0xff000000) >>24 |  (  (x1 & 0x0f000000) >>16)   )  );
   ...
   // B[0]= (uint16_t)(   (   (x & 0xff000000) >>24 |  (  (x1&0xff000000) >>16)   )  );
    //B+=48;//offset;
   *((uint16_t*)(B+48)) = (uint16_t)( ((x & 0xff0000) >>16|((x1&0x0f0000) >>8)));
   // B[48] = (uint16_t)( ((x & 0xff0000) >>16|((x1&0xff0000) >>8)));
    //+=48;//offset;
        *((uint16_t*)(B+2*48)) = (uint16_t)(((x & 0xff00) |((x1&0x0f00) <<8))>>8);
  // B[2*48]  = (uint16_t)(((x & 0xff00) |((x1&0xff00) <<8))>>8);
   // B+=48;//offset;
    *((uint16_t*)(B+3*48)) =(uint16_t)( (x & 0xff) |((x1&0x0f) <<8));
  // B[3*48] =(uint16_t)( (x & 0xff) |((x1&0xff) <<8));
     //B+=48;//offset;
   
    *((uint16_t*)(B+4*48)) = (uint16_t)(((y & 0xff000000) >>8 |((y1&0x0f000000) ))>>16);
  // B[4*48]= (uint16_t)(((y & 0xff000000) >>8 |((y1&0xff000000) ))>>16);
    
     *((uint16_t*)(B+5*48)) = (uint16_t)(((y & 0xff0000) |((y1&0x0f0000) <<8))>>16);
 //B[5*48]=  (uint16_t)(((y & 0xff0000) |((y1&0xff0000) <<8))>>16);
    
    *((uint16_t*)(B+6*48)) = (uint16_t)(((y & 0xff00) |((y1&0x0f00) <<8))>>8);
 // B[6*48]=    (uint16_t)(((y & 0xff00) |((y1&0xff00) <<8))>>8);
   
    
    *((uint16_t*)(B+7*48)) = (uint16_t)((y & 0xff) |  (  (y1 & 0x0f) << 8 ) )   ;
